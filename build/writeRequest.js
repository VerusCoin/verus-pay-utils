...
module.exports = {
  ...
  writeRequest: function (currency_id, amount, coinObj = null, note = "", compressResult = false) {
    ...
    const amountBn = new BigNumber(amount).multipliedBy(new BigNumber(10).exponentiatedBy(new BigNumber(decimals)));
    const result = VerusZkedidUtils.VerusLink.writeLink([VerusZkedidUtils.PresetObjects.VerusPaymentRequest.create(currency_id, amountBn.toString(), decimals, coinObj != null ? VerusZkedidUtils.StructuredCurrencyImport.writeImport([coinObj]) : null, note)]);
